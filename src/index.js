...
m.route(document.body, "/", {
    "/": {
        render: function () {
            return m(Layout, { title: "Repositories" }, m(RepoList))
        }
    },
    "/repo/:owner/:name": {
        render: function (vnode) {
            return m(Layout, { title: vnode.attrs.name }, m(RepoDetail, vnode.attrs))
        }
    },
    "/repo/:owner/:name/:path...": {
        render: function (vnode) {
            vnode.attrs.key = vnode.attrs.path
            return m(Layout, { title: vnode.attrs.name, header: vnode.attrs.path } , m(RepoDetail, vnode.attrs))
        }
    },
    "/edit/:owner/:name/:path...": {
        render: function (vnode) {
            return m(Layout, { title: vnode.attrs.name, header: vnode.attrs.path } , m(Editor, vnode.attrs))
        }
    },
    "/report/:owner/:name/:sha": {
        render: function (vnode) {
            return m(Layout, { title: vnode.attrs.name } , m(Report, vnode.attrs))
        }
    },
    "/about": {
        render: function () {
